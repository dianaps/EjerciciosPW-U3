...
function esHex(){
    ...
    pantalla.innerHTML=hexa.reverse().join("");
  }
...
function cuadrado(){
  pantalla.innerHTML=Math.pow(x, 2);
}
function cubo(){
  pantalla.innerHTML=Math.pow(x, 3);
}
function potencia(){
  if(x!=null){
    pantalla.innerHTML=Math.pow(ni, x);
  }
}

function operarPW(){
  potencia(); //si hay operaciones pendientes se realizan primero
  ni=x //ponemos el 1º número en "numero en espera" para poder escribir el segundo.
  xi=1; //inicializar pantalla.
}

function asbolute(){
  pantalla.innerHTML=Math.abs(x);
}
function raizsq(){
  pantalla.innerHTML=Math.sqrt(x);
}
function raizsq3(){
  pantalla.innerHTML=Math.sqrt(x, 1/3);
}
function numbE(){
  numero(Math.E);
...
function cos(){
  pantalla.innerHTML=(Math.cos(x));
}
function sen(){
  pantalla.innerHTML=(Math.sin(x));
}
function tan(){
  pantalla.innerHTML=(Math.tan(x));
}


function abre(){
  window.open('calcscy.html','','toolbar=no, status=no, resizable');
}
