		...
		if (oTbl.rows.length>1){
			for (i=1; i<=10; i++){
				...
				if (isNaN(sValorCapturado))
					...
					//se le colocaron llaves por que se realizan mas de dos operaciones dentro de if
					//via cuando es correcta la respuesta
					if (nCapturado == (nTabla*i)){
						nCalif++;
						//Obtiene el elemento por su identificador(la celda) + la posicion en la que se encuentra el contador y la pinta de verde
						document.getElementById('txtResultado'+i).style.backgroundColor = "green";
					}
					//via cuando es incorrecta la respuesta
					else{
						//Obtiene el elemento por su identificador(la celda) + la posicion en la que se encuentra el contador y la pinta de rojo
